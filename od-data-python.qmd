...
if (!file.exists("pamplona_zones.csv") || !file.exists("pamplona_real_flows.csv")) {
  locs <- readRDS('locations_north.rds')
  flows <- readRDS('flows_north_time.rds')
  locs_sf <- st_as_sf(locs, coords = c('lon', 'lat'), crs = 4326, remove = FALSE)
  centre <- st_sfc(st_point(c(-1.6432, 42.8166)), crs = 4326)
  locs_sf$dist <- as.numeric(st_distance(locs_sf, centre))
  p_zones <- locs_sf |> filter(dist <= 10000)
  write.csv(st_drop_geometry(p_zones), 'pamplona_zones.csv', row.names = FALSE)
  p_flows <- flows |>
    filter(origin %in% p_zones$id, dest %in% p_zones$id) |>
    summarise(daily_volume = sum(count) / 3, .by = c(origin, dest))
  write.csv(p_flows, 'pamplona_real_flows.csv', row.names = FALSE)
}
```
