...
# Download and crop cross-border elevation data for Spain and France
library(geodata)
library(terra)
elev_esp <- elevation_30s(country = "ESP", path = tempdir())
elev_fra <- elevation_30s(country = "FRA", path = tempdir())
elev_merged <- merge(elev_esp, elev_fra)
study_area_ext <- ext(-3.6, 4.2, 41.8, 44.0)
elev_cropped <- crop(elev_merged, study_area_ext)
writeRaster(elev_cropped, "spain_elevation.tif", overwrite = TRUE)

system("gh release list")
system("gh release upload v1 flows_north_time.parquet locations_north.parquet flows_north_time.rds locations_north.rds spain_elevation.tif --clobber")
```
...
We'll use a larger area to cover a roughly equal area of Spain and France.
...
![](images/hourly_border_flows.png)
...
In this section, we analyze the spatial relationship between the physical geography of the region (elevation) and human mobility flows. We define a Southwest-to-Northeast transect line across our study corridor, segment it into 100 equal-length chunks, extract the average elevation for each chunk from a digital elevation model (DEM), and correlate this physical feature with the aggregate travel flows in the corresponding districts.

### 1. Download and load elevation data

We use a low-resolution Digital Elevation Model (DEM) of Spain, cropped to the Northern Spain study area. The file is hosted as a GitHub Release asset for quick and reliable download.

```{r}
#| label: load-elevation-data
#| eval: false
#| echo: true
if (!file.exists("spain_elevation.tif")) {
  release_url <- "https://github.com/tdscience/tartu26/releases/download/v1/spain_elevation.tif"
  download.file(release_url, "spain_elevation.tif", mode = "wb")
}
...
elev <- rast("spain_elevation.tif")
```

### 2. Segmentize the transect line

We define the transect line from $(-3.1, 42.8)$ to $(-1.0, 43.4)$ in WGS84 coordinates. To partition it into equal-length spatial chunks, we project it to UTM Zone 30N (EPSG:25830) and sample points along it to construct 100 individual segments of equal length.
...
line <- st_linestring(matrix(c(-3.1, -1.0, 42.8, 43.4), ncol = 2)) |>
  st_sfc(crs = 4326) |>
  st_sf(geometry = _)
...
### 3. Extract elevation and associate with mobility flows

For each chunk, we calculate the average elevation from the DEM. We then identify which district/zone each chunk's centroid falls into and join the total origin trip volume (flow) for that district.
...
spod_set_data_dir('data')
zones <- spod_get_zones(zones = "distr", ver = 2)
bbox_north <- st_bbox(c(xmin = -3.1, ymin = 42.8, xmax = -1.0, ymax = 43.4), crs = 4326)
bbox_sf <- st_as_sfc(bbox_north)
zones_wgs84 <- st_transform(zones, 4326)
sf_use_s2(FALSE)
...
start_pt <- st_sfc(st_point(c(-3.1, 42.8)), crs = 4326) |> st_transform(25830)
