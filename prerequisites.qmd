...
You can also get the same 'devcontainer' environment shown in the image above running locally on your machine using VS Code and the Remote Containers extension.
This requires you to have Docker installed and running on your machine, and may require admin rights to set up (see @sec-local-docker for more details on local setup).
...
This is the most reliable way to ensure a consistent environment. The repository includes a pre-configured `.devcontainer` that sets up all dependencies (R, Python, Quarto, VS Code extensions) inside an isolated container.
